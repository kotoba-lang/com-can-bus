(ns can_bus.main
  "Kotodama WASM entrypoint for the Can_bus clean-room actor (L4) — Clojure port.
  ...
  facts (`can_bus.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "can_bus")
(def tier "L4")
...
(def entity-specs
  [{:entity "CanFrame"   :plural "canframes"   :id-prefix "canbus_can"
    :fields [:identifier :frameType :format :dlc :rtr :data :crc] :required [:identifier :frameType]
    :coerce {:identifier :int :dlc :int :crc :int :rtr :bool} :refs {}}
   {:entity "CanFdFrame" :plural "canfdframes" :id-prefix "canbus_can"
    :fields [:identifier :frameType :format :dlc :brs :esi :data :crc] :required [:identifier :frameType]
    :coerce {:identifier :int :dlc :int :crc :int :brs :bool :esi :bool} :refs {}}
   {:entity "ErrorFrame" :plural "errorframes" :id-prefix "canbus_err"
    :fields [:errorType :errorFlag :transmitErrorCounter :receiveErrorCounter :nodeState] :required [:errorType :errorFlag]
    :coerce {:transmitErrorCounter :int :receiveErrorCounter :int} :refs {}}
   {:entity "RemoteFrame" :plural "remoteframes" :id-prefix "canbus_rem"
    :fields [:identifier :format :dlc :rtr] :required [:identifier :format]
    :coerce {:identifier :int :dlc :int :rtr :bool} :refs {}}
   {:entity "OverloadFrame" :plural "overloadframes" :id-prefix "canbus_ove"
    :fields [:overloadFlag :timestamp] :required [:overloadFlag :timestamp]
    :coerce {} :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"can_bus.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "can_bus-compat" :tier tier :entities entities} 200])
...
(defn start! [] :can_bus-compat/ready)
